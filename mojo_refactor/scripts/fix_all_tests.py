#!/usr/bin/env python3
"""
Script to properly fix all Mojo test files.
"""
...
def fix_test_file(filepath):
    ...
    # Fix broken patterns like ")==" or ")!=" or "!)="
    content = re.sub(r'\s*\)==', ' ==', content)
    content = re.sub(r'\s*\)!=', ' !=', content)
    content = re.sub(r'\s*!\)=', ' !=', content)
    content = re.sub(r'\s*\)=', ' =', content)
    
    # Fix lines ending with incomplete assert_equal or assert_true
    lines = content.split('\n')
    fixed_lines = []
    for i, line in enumerate(lines):
        # Fix unclosed parenthesis in assert_equal or assert_true
        if 'assert_equal(' in line or 'assert_true(' in line or 'assert_false(' in line:
    ...
    # Fix remaining assert statements that should use assert_true
    # Pattern: assert XXX == YYY (not inside assert_equal)
    content = re.sub(r'^(\s*)assert\s+(\S+)\s*==\s*(\S+)\s*$', r'\1assert_equal(\2, \3)', content, flags=re.MULTILINE)
    content = re.sub(r'^(\s*)assert\s+(\S+)\s*!=\s*(\S+)\s*$', r'\1assert_true(\2 != \3)', content, flags=re.MULTILINE)
    content = re.sub(r'^(\s*)assert\s+(.+)$', r'\1assert_true(\2)', content, flags=re.MULTILINE)
