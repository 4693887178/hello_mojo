...
def check_string(value: String, name: String) raises -> Bool:
    if len(value) == 0:
        raise Error(gettext("Argument '") + name + gettext("' cannot be empty string"))
    return True


def check_int(value: Int, name: String, min_val: Int = 0, max_val: Int = 999999999) raises -> Bool:
    if value < min_val:
        raise Error(gettext("Argument '") + name + gettext("' must be >= ") + String(min_val))
    if value > max_val:
        raise Error(gettext("Argument '") + name + gettext("' must be <= ") + String(max_val))
    return True
...
def check_percentage(value: Float64, name: String) raises -> Bool:
    if value < 0.0 or value > 1.0:
        raise Error(gettext("Argument '") + name + gettext("' must be between 0 and 1"))
    return True


def check_order_book_id(value: String, name: String) raises -> Bool:
    if len(value) == 0:
        raise Error(gettext("Argument '") + name + gettext("' cannot be empty"))
    if value.find(".") < 0:
        raise Error(gettext("Argument '") + name + gettext("' must be in format 'CODE.EXCHANGE'"))
    return True
