...
Key differences from Python original:
  ...
  - round_method uses String ("floor"/"ceil"/"round") instead of Python Callable
"""
...
def _get_order_style_price(env: Environment, order_book_id: String, style: OrderStyle) -> Float64:
    ...
    if style.style_type == ORDER_TYPE.MARKET:
...
def _order_value(
    ...
    elif style.style_type == ORDER_TYPE.ALGO:
        price = env.get_last_price_from_proxy(order_book_id)
        if not _is_valid_price(price):
            env.order_creation_failed(order_book_id, "No valid algo price")
            return None
    else:
        price = env.get_last_price_from_proxy(order_book_id)
        if not _is_valid_price(price):
...
def order_lots(
    ...
    var oroder_book_id = id_or_ins
    var ins = env.get_instrument(oroder_book_id)
    var round_lot = ins.round_lot()
    if round_lot <= 0:
        round_lot = 100
    var account_result = _get_account_position(env, oroder_book_id)
    return _order_shares(
        env, oroder_book_id, amount * round_lot, style, account_result.position_quantity, True
    )
...
def order_target_portfolio(
    ...
    if total_percent > 1.0:
        raise Error("total percent should be lower than 1, current: " + String(total_percent))
    ...
    if total_percent >= 0.999999 and total_percent <= 1.000001:
        var estimate_transaction_cost = 0.0
        for entry in target_data.items():
            var tpd = entry.value
            var current_val = Float64(current_quantities.get(tpd.order_book_id, 0)) * tpd.last_price
            var change_val = target_portfolio[tpd.order_book_id] * account_value - current_val
            if tpd.last_price > 0.00001:
                estimate_transaction_cost += _estimate_transaction_cost(env, tpd.instrument, Int(change_val / tpd.last_price), tpd.last_price)
