...
@always_inline
def c_localtime(var tv_sec: Int) -> CTm:
    """Wrapper for the C function localtime."""
    var tm = CTm()
    _ = external_call["localtime_r", NoneType](
        UnsafePointer(to=tv_sec), UnsafePointer(to=tm)
    )
    return tm
...
def c_strptime(time_str: String, time_format: String) -> CTm:
    ...
    var tm = CTm()
    _ = external_call["strptime", NoneType](
        time_str.unsafe_ptr(), time_format.unsafe_ptr(), UnsafePointer(to=tm)
    )
    return tm


@always_inline
def c_gmtime(var tv_sec: Int) -> CTm:
    """Wrapper for the C function gmtime."""
    var tm = CTm()
    _ = external_call["gmtime_r", NoneType](
        UnsafePointer(to=tv_sec), UnsafePointer(to=tm)
    )
    return tm
