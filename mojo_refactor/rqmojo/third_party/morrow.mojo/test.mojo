...
def assert_datetime_equal(dt: Morrow, py_dt: PythonObject) raises:
    assert_true(
        dt.year == Int(py=py_dt.year)
        and dt.month == Int(py=py_dt.month)
        and dt.hour == Int(py=py_dt.hour)
        and dt.minute == Int(py=py_dt.minute)
        and dt.second == Int(py=py_dt.second),
...
def test_strptime() raises:
    ...
    assert_equal(String.write(m), "2023-01-20T15:49:10.000000+00:00")

    m = Morrow.strptime("2023-10-18 15:49:10 +0800", "%Y-%m-%d %H:%M:%S %z")
    assert_equal(String.write(m), "2023-10-18T15:49:10.000000+08:00")

    m = Morrow.strptime("2023-10-18 15:49:10", "%Y-%m-%d %H:%M:%S", "+09:00")
    assert_equal(String.write(m), "2023-10-18T15:49:10.000000+09:00")
