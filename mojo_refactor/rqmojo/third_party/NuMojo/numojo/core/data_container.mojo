...
# temporary DataContainer to support transition from UnsafePointer to UnsafePointer.
...
struct DataContainer[dtype: DType](ImplicitlyCopyable):
    ...
    fn __init__(out self, ptr: UnsafePointer[Scalar[Self.dtype], MutExternalOrigin]):
        """
        Do not use this if you know what it means.
        If the pointer is associated with another array, it might cause
        dangling pointer problem.
    ...
    fn get_ptr(self) -> UnsafePointer[Scalar[Self.dtype], MutExternalOrigin]:
        return self.ptr
