...
from std.memory import memcpy, memcmp
...
struct NDArrayShape(
    ImplicitlyCopyable, Movable, Writable, Sized, RegisterPassable
):
    """
    ...
    # Fields
    var _buf: UnsafePointer[Scalar[Self.element_type], MutExternalOrigin]
    """Data buffer."""
    var ndim: Int
    """Number of dimensions of array. It must be larger than 0."""
    ...
    @always_inline("nodebug")
    fn __init__(out self, shape: VariadicList[Int, _], size: Int) raises:
        """
        Initializes the NDArrayShape with a list of shape dimensions and a specified size.
    ...
    fn __init__(out self, *, copy: Self):
        ...
        Args:
            copy: Another NDArrayShape to initialize from.
        """
        self.ndim = copy.ndim
        if copy.ndim == 0:
