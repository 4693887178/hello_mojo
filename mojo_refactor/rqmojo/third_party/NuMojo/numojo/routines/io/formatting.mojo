...
fn format_floating_scientific[
    ...
    try:
        ...
        var result: String
        if x < 0:
            result = "-" + mantissa_without_sign_string[byte=0: 2 + precision]
        else:
            if sign:
                result = "+" + mantissa_without_sign_string[byte=0: 2 + precision]
            else:
        ...
        if suppress_scientific and abs(power) <= exponent_threshold:
            var formatted = format_floating_precision(x, precision, sign)
            var padding = formatted_width - len(formatted)
            if padding > 0:
        ...
        var formatted_result = String("{0}{1}").format(result, exponent_string)
        var padding = formatted_width - len(formatted_result)
        if padding > 0:
            return " " * padding + formatted_result
