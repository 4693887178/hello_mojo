...
from rqmojo.const import RUN_TYPE, DEFAULT_ACCOUNT_TYPE
...
def test_scheduler_weekly_all_weekdays() raises:
    ...
    assert_equal(len(scheduler._registry), 5)
...
def test_scheduler_clear() raises:
    ...
    scheduler.clear()
    assert_equal(len(scheduler._registry), 0)
...
def test_scheduler_mixed_schedules() raises:
    ...
    assert_equal(len(scheduler._registry), 4)

    print("  Mixed schedules:")
    for i in range(len(scheduler._registry)):
        var entry = scheduler._registry[i]
...
def test_run_monthly_simulation() raises:
    ...
    print("  Scheduler has " + String(len(scheduler._registry)) + " scheduled function(s)")
