"""
Comprehensive test suite for rqmojo/mod/rqmojo_mod_sys_accounts/api/api_stock.mojo

Tests all public API functions:
  - _round_order_quantity: KSH/BJS/normal board handling, round methods
  - _is_nan / _is_valid_price: NaN and price validation
  - _get_account_position: Position data extraction
  - stock_order_shares: Buy/sell orders by share count
  - stock_order_lots: Order by lot count
  - stock_order_value: Order by cash amount
  - stock_order_percent: Order by portfolio percentage
  - stock_order_target_value: Target position value
  - stock_order_target_percent: Target position percent
  - stock_order / stock_order_to: Wrapper functions
  - order_target_portfolio: Multi-asset rebalancing
  - is_suspended / is_st_stock: Status queries
  - industry / sector: Classification lookups
  - get_dividend: Dividend info retrieval
"""
...
from rqmojo.model.order import OrderStyle, MarketOrder, LimitOrder, create_order_with_id
from rqmojo.model.instrument import Instrument, create_stock_instrument
from rqmojo.const import SIDE, POSITION_EFFECT, ORDER_TYPE, INSTRUMENT_TYPE, EXCHANGE, MARKET, POSITION_DIRECTION
...
def _create_cs_instrument(order_book_id: String = "000001.XSHE", round_lot_val: Int = 100) -> Instrument:
...
def test_industry_returns_list() raises:
    ...
    assert_true(isinstance(result, List[String]))
