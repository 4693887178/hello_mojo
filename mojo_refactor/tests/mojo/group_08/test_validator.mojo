...
Comprehensive tests for MarginInstrumentValidator
"""
...
def test_margin_instrument_validator_init() raises:
    var validator = MarginInstrumentValidator()
...
def test_validate_submission_reason_contains_order_book_id() raises:
    ...
    assert_true(reason.contains("600000.XSHG"), "reason contains order_book_id")
...
def test_validate_submission_reason_contains_order_creation_failed() raises:
    ...
    assert_false(result is None, "result should not be None")
    var reason = result.value()
    assert_true(reason.contains("Order Creation Failed"), "reason contains 'Order Creation Failed'")
...
def test_validate_submission_reason_contains_cash_liabilities() raises:
    ...
    assert_false(result is None, "result should not be None")
    var reason = result.value()
    assert_true(reason.contains("cash liabilities > 0"), "reason contains 'cash liabilities > 0'")
...
def test_validator_with_different_order_book_ids() raises:
    ...
    assert_true(result2.value().contains("600000.XSHG"), "reason contains correct order_book_id")
