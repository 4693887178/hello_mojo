...
- **环境单例**：通过`Environment.get_instance()`中央注册表访问所有组件
- **事件驱动**：模块订阅事件（BAR、TICK、BEFORE\_TRADING 等）。
- **模组系统**：通过`AbstractMod`接口进行扩展
- **数据包**：HDF5 格式，存储在`~/.rqalpha/bundle/`
- **配置层级**：CLI 参数 > 策略`__config__`> 配置文件 > 默认值
...
3. **Strategy Execution** (`rqalpha/core/`)
   ...
   - `Executor`: Executes strategy lifecycle (init, before\_trading, handle\_bar, etc.)
